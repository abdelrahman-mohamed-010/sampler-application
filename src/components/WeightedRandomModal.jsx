  ...
  const handleProceed = () => {
    ...
    function weightedRandomSample(pop, size) {
      ...
      while (sample.length < size && populationCopy.length > 0) {
        const totalWeight = populationCopy.reduce((sum, row) => sum + (parseFloat(row["AMOUNT"]) || 1), 0);
  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50">
      ...
      >
        ...
        <h2 className="mb-8 text-center text-2xl text-dark font-bold">Weighted Random Sample</h2>
        <div className="flex flex-col items-center gap-6 pb-7 border-dark border-b-2 w-[90%] mx-auto">
          ...
          <div className="flex items-center gap-2">
            <span className="text-dark font-semibold text-lg">Sample Size:</span>
            <input
              type="number"
              value={sampleSize}
